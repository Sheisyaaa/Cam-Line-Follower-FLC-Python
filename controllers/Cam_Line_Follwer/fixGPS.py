...
while robot.step(timestep) != -1:
    ...
    if len(contours) > 0:
        ...
        print(f"Motor Kiri: {motor_kiri:.2f} || Motor Kanan: {motor_kanan:.2f} || Eror: {error} || Delta Eror: {delta_err}      ||      GPS X: {gpsX:.2f} || GPS Y: {gpsY:.2f} || GPS Z: {gpsZ:.2f}")
        # print(f'GPS X: {gpsX:.2f} || GPS Y: {gpsY:.2f} || GPS Z: {gpsZ:.2f}')
        # print(f"{error}")
        # print(f"{delta_err}")
        leftMotor.setVelocity(motor_kiri)
        rightMotor.setVelocity(motor_kanan)
        # save_gps_data(gpsX, gpsY, gpsZ)
        save_gps_data = [gpsX, gpsZ]
        save_to_csv(save_gps_data)
